...
export function LeafletMap({ lat, lon, className, onMapReady }: LeafletMapProps) {
  ...
  // Fly to new coordinates when props change after initial mount
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    async function init() {
      ...
      // Guard against Strict Mode double-invocation: the cleanup may have run
      // (and removed the map) while the async import was in flight.
